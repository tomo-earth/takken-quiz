  ...
  <main>
    ...
    <section class="screen" id="scr-add">
      ...
      <div id="newKoushuBox" hidden>
        <label style="color:var(--orange-ink)">⚠ この工種は初めてです。どの工程に入れるか選んでください（必須）</label>
    ...
    <section class="screen" id="scr-name">
      <div class="banner">はじめまして。あなたのお名前をタップしてください</div>
      <div class="chips" id="nameChips"></div>
      <div class="hint">一度登録すると次回からは聞きません（台帳の「設定」シートに保存されます）。</div>
    </section>
...
google.script.run.withSuccessHandler(function(d){
  ...
  $('aParent').innerHTML = '<option value="">▼ 工程を選んでください</option>' +
    d.kouTei.map(function(v){ return '<option>' + esc(v) + '</option>'; }).join('');
  ...
  restoreDraft();
  if (!d.userName) { askName(); } else { show('home'); }
...
/* ---------- 初回の名前登録 ---------- */
function askName(){
  var el = $('nameChips'); el.innerHTML = '';
  INIT.workers.forEach(function(w){
    var c = document.createElement('span'); c.className = 'chip'; c.textContent = w;
    c.onclick = function(){
      el.querySelectorAll('.chip').forEach(function(x){ x.classList.remove('on'); });
      c.classList.add('on');
      google.script.run.withSuccessHandler(function(name){
        INIT.userName = name; $('barUser').textContent = name + 'さん';
        toast(name + 'さんとして登録しました'); show('home');
      }).withFailureHandler(function(e){ toast('登録できませんでした：' + e.message, true); })
      .api_setMyName(w);
    };
    el.appendChild(c);
  });
  document.querySelectorAll('.screen').forEach(function(s){ s.classList.remove('on'); });
  $('scr-name').classList.add('on'); $('barTitle').textContent = 'お名前の登録'; $('backBtn').hidden = true;
}
...
function saveWork(){
  ...
  if (isNew && !$('aParent').value){
    $('newKoushuBox').hidden = false;
    return toast('「' + koushu + '」は新しい工種です。どの工程かを選んでください。', true);
  }
